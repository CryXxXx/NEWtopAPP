...
// Проверяем, есть ли аватарка и никнейм
if (user) {
    // Создаем элемент img с классом avatar
    const img = document.createElement('img');
    img.src = user.photo_url; // Устанавливаем URL аватарки
    img.alt = 'Avatar'; // Альтернативный текст
    img.className = 'user-img'; // Применяем класс для стилей

    // Добавляем img в контейнер для изображения
    document.getElementById('user-img-cont').appendChild(img);

    const usernameNameDiv = document.getElementById('username-name');
    usernameNameDiv.textContent = user.first_name ? user.first_name : 'Noname1';
