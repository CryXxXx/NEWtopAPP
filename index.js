...
window.addEventListener('load', () => {
    ...
    // Устанавливаем таймер, чтобы подождать пока все элементы загружены
    setTimeout(() => {
        loader.style.opacity = '0'; // Уменьшаем непрозрачность до нуля
        loader.addEventListener('transitionend', () => {
            loader.style.display = 'none'; // Скрываем элемент после завершения анимации
        });
    }, 100); // Можно настроить задержку перед исчезновением
});
...
function place_coef() {
    ...
    fetch("https://tesgdl-176-118-32-227.ru.tuna.am/get_last_game", {
